...
<%
	if ( request.getParameter( "action" ).equals( "Valider" ) )
		response.sendRedirect(plu.getConfirmModifyAtome(request));
	else if ( request.getParameter( "action" ).equals( I18nService.getLocalizedString( "plu.modify_atome.buttonReturn", request.getLocale( ) ) ) )
	{
		response.sendRedirect(plu.getConfirmCancelModifyAtome(request));
	}
	else {
%>
